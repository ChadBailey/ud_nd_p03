# level_generator.py made by Chad Bailey 4-15-2017
#
...
levels_dict['levels'] = {}
levels_dict['settings']['version'] = 1
...
for current_level in range(1,number_of_levels + 1):
	...
	while not valid:
		...
		if number_of_blanks > 0:
			...
			levels_dict['levels'][current_level] = {}
			# Create sub-dict for the answers
			levels_dict['levels'][current_level]['answers'] = {}
			# Add sentence to levels dictionary as key '1_challenge'
			levels_dict['levels'][current_level]['challenge'] = level_sentence if not enable_encoding else base64.b64encode(level_sentence)
			for current_blank in range(0,number_of_blanks):
				...
				levels_dict['levels'][current_level]['answers'][replacements_list[current_blank]] = blank_answer if not enable_encoding else base64.b64encode(blank_answer)
